...
if [[ $prep == true ]]; then
  ...
  mountdir=$(mktemp -d)
  mount "$loopback" "$mountdir"
  rm -rvf $mountdir/var/cache/apt/archives/* $mountdir/var/lib/dhcpcd5/* $mountdir/var/log/!\(zabbix-agent*\) $mountdir/var/tmp/* $mountdir/tmp/*
  find $mountdir/data/* -depth -not -path "*/REPO-INF*" -type f -exec rm -vf {} +
  find $mountdir/data/* -depth -not -path "*/REPO-INF*" -type d -delete
  umount "$mountdir"
fi
